...
#ifndef Qrap_cRapSites_h
#define Qrap_cRapSites_h
...
namespace Qwireless
{
	 ...
	 */
	class cRapSites : public QWidget
	{
			Q_OBJECT
		...
		public:
			...
			/**
			 * This function returns the currentSiteStatus.
			 */
			QString getCurrentSiteStatus ()
			{
				return mCurrentSiteStatus;
			}
			...
			/**
			 * This function is used when a site is selected from the GUI
			 */
			void EditSite (int mid);

			void SetCurrentSiteID(int CurrentSiteId) {mCurrentSiteId=CurrentSiteId;}
		...
		private slots:
			...
			/**
			 * Private SLOT - This slot is called whenever a site selection is changed, it will then 
			 * clear all the tables data and load all the data that is related to that particular site.
			 */
		...
		private:
			...
			cRapTableTab*			mSiteTable;		///< A pointer to a RapTableTab class that stores the site table.

			int				mCurrentSiteId;		///< An integer that stores the current ID of the currently selected site.
			QString				mCurrentSiteStatus;	///< A QString that holds the currently selected site status.
